#!/bin/bash
# Ultra-fine search with very small increments to find exact limit
...
echo -e "${BLUE}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  Ultra-Fine Search - Testing Tiny Increments                 ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
# Function to monitor training (faster timeout for quick tests)
monitor_training() {
    local timeout=120  # 2 minutes for quick tests
    ...
    while [ $(($(date +%s) - start_time)) -lt $timeout ]; do
        ...
        if [ -n "$current_memory" ]; then
            ...
            if [ $stable_checks -ge $stable_threshold ] && [ $checks -ge 4 ]; then
...
# Test very small increments for prefetch
echo -e "${BLUE}Testing Prefetch with tiny increments (MaxLive: ${BASELINE_MAX_LIVE})${NC}"
echo ""
...
# Test very small increments for max_live
echo -e "${BLUE}Testing MaxLive with tiny increments (Prefetch: ${OPTIMAL_PREFETCH})${NC}"
echo ""
...
# Final update
echo -e "${GREEN}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  Ultra-Fine Optimal Settings Found                            ║${NC}"
echo -e "${GREEN}╚══════════════════════════════════════════════════════════════╝${NC}"
echo ""
echo "Optimal values:"
...
echo -e "${GREEN}✓ Config updated with ultra-fine optimal values!${NC}"
