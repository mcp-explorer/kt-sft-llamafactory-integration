#!/bin/bash
# Refined binary search - tests intermediate values between working and failing
...
echo -e "${BLUE}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  Refined Binary Search - Testing Intermediate Values         ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════════════════════════╝${NC}"
echo ""

# Known values
WORKING_PREFETCH=1.2e9
WORKING_MAX_LIVE=8e9
FAILING_PREFETCH=1.5e9
FAILING_MAX_LIVE=9e9
...
echo -e "${BLUE}Testing intermediate values between working and failing...${NC}"
echo ""

# Test values: try smaller increments
TEST_VALUES=(
    "1.3e9 8.3e9"
    "1.35e9 8.5e9"
    "1.4e9 8.7e9"
...
OPTIMAL_PREFETCH=$WORKING_PREFETCH
OPTIMAL_MAX_LIVE=$WORKING_MAX_LIVE
OPTIMAL_MEMORY=0
...
for test_val in "${TEST_VALUES[@]}"; do
    ...
    echo "  Prefetch: $test_prefetch"
    echo "  Max Live: $test_max_live"
    echo ""
