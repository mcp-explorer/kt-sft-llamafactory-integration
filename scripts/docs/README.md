...
- [Synthetic Data Generation System](#synthetic-data-generation-system) - Industrial-grade SFT data generation
...
| `sft_data/generate_with_kllama.sh` | Industrial synthetic data generation | To generate high-quality, diverse SFT training data with schema/judge methods |
...
./scripts/inference/infer_ds2_chat_lite.sh webchat

# OpenAI-style API server
API_PORT=8000 ./scripts/inference/infer_ds2_chat_lite.sh api
```
...
./scripts/inference/infer_ds2_chat_lite.sh chat
# or simply:
./scripts/inference/infer_ds2_chat_lite.sh
```
...
./scripts/inference/infer_ds2_chat_lite.sh chat 10

# Use checkpoint-5 for webchat
./scripts/inference/infer_ds2_chat_lite.sh webchat 5

# Use checkpoint-11 for API
./scripts/inference/infer_ds2_chat_lite.sh api 11
```
...
# Interactive CLI chat (default)
./scripts/inference/infer_ds2_chat_lite_raw.sh chat

# Web UI chat interface
./scripts/inference/infer_ds2_chat_lite_raw.sh webchat

# OpenAI-style API server
API_PORT=8000 ./scripts/inference/infer_ds2_chat_lite_raw.sh api
```
...
# OpenAI-style API server
API_PORT=8000 ./scripts/inference/infer_ds2_chat_lite_hf.sh api
```
...
# Interactive CLI chat (default)
./scripts/inference/infer_ds2_chat_lite_raw_hf.sh chat

# Web UI chat interface
./scripts/inference/infer_ds2_chat_lite_raw_hf.sh webchat

# OpenAI-style API server
API_PORT=8000 ./scripts/inference/infer_ds2_chat_lite_raw_hf.sh api
```
...
./scripts/utils/create_identity_dataset.sh alice 2025-12-25

# Full customization
./scripts/utils/create_identity_dataset.sh bob 2024-06-15 identity_bob.json 500 300

# Show help
./scripts/utils/create_identity_dataset.sh --help
```
...
The `sft_data/` directory contains an industrial-grade synthetic data generation system for creating high-quality SFT training datasets. This system uses advanced techniques like schema-driven generation (DSPy-style) and judge-based filtering to produce diverse, consistent training data.

### Quick Start

```bash
cd /home/sean/Documents/ktransformers/sft_data

# Activate environment and generate data in one command
./generate_with_kllama.sh \
    --goal "identity training" \
    --num_records 1000 \
    --constraint identity=sean \
    --constraint date=2026-01-01 \
    --method hybrid \
    --provider gemini
```
...
# Generate with constraints as key-value pairs (recommended)
./generate_with_kllama.sh \
    --goal "identity training" \
    --num_records 1000 \
    --constraint identity=sean \
...
# Or use JSON constraints (alternative)
./generate_with_kllama.sh \
    --goal "identity training" \
    --num_records 1000 \
...
```bash
# Only activate environment and install requirements (no generation)
./generate_with_kllama.sh --install
```

### What It Does

The `generate_with_kllama.sh` script:

1. **Activates Kllama Environment**: Automatically activates the `Kllama` conda environment (creates it if missing)
2. **Installs Requirements**: Checks and installs required packages (`pydantic`, `openai`, `anthropic`, `google-generativeai`, `python-dotenv`)
3. **Loads Environment Variables**: Automatically loads API keys from `.env` file in project root
4. **Runs Generation**: Executes the Python generation script with all provided arguments

### Generation Methods

The system supports three generation methods:

#### 1. Schema-Driven (DSPy-style) ⭐⭐⭐⭐⭐
...
- **Benefits**: High data consistency, controllable components, perfect for structured output
- **Use Case**: When you need specific formats, reasoning chains, or structured responses

#### 2. Judge-Based (Critic Pipeline) ⭐⭐⭐⭐⭐
...
- **Core Idea**: Schema generation + Judge validation
- **Benefits**: Combines format compliance (schema) with quality filtering (judge)
- **Use Case**: Best of both worlds - consistent format and high quality

### Constraint Arguments

You can pass constraints in two ways:
...
- **Easy to use**: No JSON escaping needed
- **Multiple constraints**: Use `--constraint` multiple times
- **Type inference**: Automatically parses numbers, booleans, arrays from JSON

#### JSON String (Alternative)

```bash
--constraints '{"identity": "sean", "date": "2026-01-01", "max_length": 500}'
```

- **Complex structures**: Better for nested objects or arrays
- **Single argument**: All constraints in one string

### Command-Line Arguments

| Argument | Type | Required | Description |
|----------|------|----------|-------------|
| `--goal` | string | Yes | Goal description (e.g., "identity training", "math problem solving") |
| `--num_records` | int | Yes | Number of records to generate |
| `--method` | choice | No | Generation method: `schema`, `judge`, or `hybrid` (default: `hybrid`) |
| `--provider` | choice | No | LLM provider: `openai`, `anthropic`, or `gemini` (default: `openai`) |
| `--constraint` | KEY=VALUE | No | Add a constraint (can be used multiple times) |
| `--constraints` | JSON | No | JSON string of constraints (alternative to `--constraint`) |
| `--model` | string | No | Model name (optional, uses provider default) |
| `--output` | path | No | Output file path (default: `outputs/goal_timestamp.jsonl`) |
| `--score_threshold` | float | No | Minimum quality score for judge filtering (default: 0.7) |
| `--target_format` | choice | No | Data format: `instruction`, `conversation`, `qa`, `reasoning` (default: `instruction`) |

### API Keys

The system supports multiple LLM providers. API keys are loaded from:

1. **`.env` file** (recommended): Automatically loaded from project root
   ```bash
...
- **Samples**: `sft_data/outputs/goal_timestamp.jsonl` (JSONL format)
- **Metadata**: `sft_data/outputs/goal_timestamp_metadata.json` (generation stats)

Each sample includes:
- `instruction`: The input/question
- `output`: The expected response
- `input`: Optional additional context
- `metadata`: Generation method, validation status, etc.
- `score`: Quality score (if using judge method)
...
cat outputs/identity_training_*.jsonl | head -5

# 3. Convert to LLaMA-Factory format (if needed)
# See sft_data/README.md for conversion scripts

# 4. Use in training
# Update LLaMA-Factory/data/dataset_info.json to register the dataset
# Then use it in your training config: dataset: your_dataset_name
```
...
./generate_with_kllama.sh \
    --goal "identity training" \
    --num_records 1000 \
    --output custom/path/my_data.jsonl \
    --method hybrid \
    --provider gemini
```
...
- **Overfitting Issues**: Generate diverse, high-quality data to prevent overfitting
- **Data Scarcity**: Create synthetic data when real data is limited
- **Consistency Requirements**: Use schema-driven generation for structured outputs
- **Quality Control**: Use judge-based filtering for critical applications
- **Rapid Prototyping**: Quickly generate test datasets for experimentation

### Integration with Training

After generating data:

1. **Register Dataset**: Add to `LLaMA-Factory/data/dataset_info.json`
2. **Convert Format**: Ensure data matches LLaMA-Factory's expected format
3. **Train**: Use the dataset in your training config

See `sft_data/README.md` for detailed integration instructions.
...
- Verify provider supports the model you're using
- Check internet connection for API calls
- Review error messages in output

### More Information

- **Full Documentation**: See `sft_data/README.md` for architecture details
- **Quick Start**: See `sft_data/QUICK_START.md` for examples
- **Architecture**: See `sft_data/ARCHITECTURE.md` for system design
...
**Solution**: 
- For API: `API_PORT=8001 ./scripts/inference/infer_ds2_chat_lite.sh api`
...
# Option A: Simple identity dataset
./scripts/utils/create_identity_dataset.sh sean 2026-01-01

# Option B: High-quality synthetic data (recommended)
cd sft_data
./generate_with_kllama.sh \
    --goal "identity training" \
    --num_records 1000 \
...
./scripts/inference/infer_ds2_chat_lite.sh chat        # Fine-tuned
./scripts/inference/infer_ds2_chat_lite_raw.sh chat    # Raw

# Compare fine-tuned vs raw model (HuggingFace)
./scripts/inference/infer_ds2_chat_lite_hf.sh chat     # Fine-tuned
./scripts/inference/infer_ds2_chat_lite_raw_hf.sh chat # Raw

# Compare backends (fine-tuned)
./scripts/inference/infer_ds2_chat_lite.sh chat        # KTransformers
./scripts/inference/infer_ds2_chat_lite_hf.sh chat    # HuggingFace
```
...
- **Synthetic Data Generation**: See [Synthetic Data Generation System](#synthetic-data-generation-system) section above
- **LLaMA-Factory Usage**: See `scripts/USAGE_GUIDE.md` for detailed LLaMA-Factory workflows
- **Training Configs**: See `LLaMA-Factory/examples/train_lora/` for training configuration examples
- **Inference Configs**: See `LLaMA-Factory/examples/inference/` for inference configuration examples
