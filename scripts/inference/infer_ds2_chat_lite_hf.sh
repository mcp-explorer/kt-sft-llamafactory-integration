#!/bin/bash
# Run inference for DeepSeek-V2-Lite-Chat fine-tuned model using HuggingFace backend
# This script supports chat, webchat, and API modes
# Uses adapters trained with ktransformers (but saved in standard PEFT format)
...
INFERENCE_CONFIG="$PROJECT_ROOT/LLaMA-Factory/examples/inference/deepseek2_lite_inference_hf.yaml"
...
if [ -n "$CHECKPOINT" ] && [ -d "$ADAPTER_PATH" ]; then
    ...
    TEMP_CONFIG="/tmp/deepseek2_lite_inference_hf_$$.yaml"
